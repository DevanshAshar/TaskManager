...
const joinGrp=async(req,res)=>{
    try {
        ...
        const grp=await Group.findById(grpid)
        if(!grp)
        return res.status(400).json({message:'No such grp found'})
...
const updateGrpTask=async(req,res)=>{
    try {
       const {grpid,title,description,status,deadline,taskFor,tid}=req.body 
       const grp=await Group.findById(grpid)
...
const myGrps=async(req,res)=>{
    try {
        const grps=await User.findById(userData._id).populate('grps')
...
module.exports={createGrp,joinGrp,deleteGrp,updateGrpTask,leaveGrp,addGrpTask,deleteGrpTask,remove,myGrps}